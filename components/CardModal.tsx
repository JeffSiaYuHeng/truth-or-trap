...
const CardModal: React.FC = () => {
    ...
    if (isKingModalOpen) return <KingCardModal isOpen={true} onClose={handleSubModalClose} />;
    if (isBattleSetupOpen) return <BattleSetupModal onClose={handleSubModalClose} />;
    if (isStealSetupOpen) return <StealSetupModal onClose={handleSubModalClose} />;
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={handleClose}>
            <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl p-6 space-y-4 border border-gray-200" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center">
                    <h2 className="text-3xl font-bold text-[#FAB655]">{t('myCards')}</h2>
                    <button onClick={handleClose} className="text-gray-400 hover:text-gray-800 text-3xl font-light">&times;</button>
                </div>
                ...
                {currentPlayer.cards.length > 0 ? (
                    <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                        {currentPlayer.cards.map((card, index) => {
                            ...
                            return (
                                <div key={`${card}-${index}`} className="bg-gray-100 p-3 rounded-lg flex items-center justify-between gap-4">
                                    <img src={cardDetails.image} alt={cardLocalization.name} className="w-12 h-12 flex-shrink-0" />
                                    <div className="flex-grow">
                                        <h3 className="font-bold text-gray-800 text-lg">{cardLocalization.name}</h3>
                                        <p className="text-gray-600 text-sm">{cardLocalization.description}</p>
                                    </div>
                                    <button 
